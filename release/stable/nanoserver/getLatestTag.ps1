...
if(!$CI.IsPresent)
{
    Get-DockerTags -ShortTags $shortTags -Image "microsoft/nanoserver" -FullTagFilter '\d{4}_KB\d{7}' -Mcr
}
else {
    ...
    Get-DockerTags -ShortTags $shortTags -Image "microsoft/nanoserver" -FullTagFilter '10\.0\.14393\.\d{4,}$' -SkipShortTagFilter -Mcr
}
